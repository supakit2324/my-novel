...
const NovelEditDescription = ({props}: IProps) => {
  console.log(props)
  ...
  const [synopsis, setSynopsis] = useState<string>(props.synopsis || "");
  ...
  const initialLanguages = languageOptions.filter((option) =>
      props.language.includes(option.value)
  );

  const selectedCategoryTagsOptions = Object.values(tagsInTagSections)
      .flat()
      .filter((option) => props.categoryTags?.includes(option.value));
  ...
  useEffect(() => {
    ...
    setSelectedCategory(
        categoryOptions.find((option) => option.value === props.type) || null
    );
    setSelectedLengthStory(
        lengthStoryOptions.find((option) => option.value === props.length) || null
    );
    setSelectedGender(
        primaryGender.find((option) => option.value === props.primaryGender) || null
    );
    setSelectedTag(
        tags.find((option) => option.value === props.tags) || null
    );
    setSelectedContentRating(
        contentRatingOptions.find((option) => option.value === props.contentRating) || null
    )
    setSelectedBookType(
        bookTypeOptions.find((option) => option.value === props.bookType) || null
    )
    setSelectedPriceOption(
        setPriceOptions.find((option) => option.value === props.isFree) || null
    )
    setSelectedLanguages(initialLanguages);
    setSelectedCategoryTags(selectedCategoryTagsOptions);

    if (props.bookType === "Single Volume Type") {
      setIsPriceSet(true);
    } else {
      setIsPriceSet(false);
    }
  }, [
    props.title, props.synopsis, props.abbreviation, props.section, props.type,
    props.primaryGender, props.language, props.tags, props.categoryTags,
    props.length, props.contentRating, props.bookType, props.isFree, props.price
  ]);
  ...
  const handleBookTypeChange = (selectedOption: SingleValue<OptionType>) => {
    ...
    if (selectedOption?.value === "Single Volume Type") {
      setIsPriceSet(true);
    } else {
      setIsPriceSet(false);
    }
  ...
  const handlePriceChangeInput = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        <div className="row">
          ...
          <div className="col-sm-12">
            <div className="mb20">
              ...
              <input type="text" className="form-control" value={abbreviation} placeholder="No more than 15 characters"/>
            </div>
          </div>
          ...
          <div className="col-sm-12">
            <div className="mb20">
              ...
              <input type="text" className="form-control" value={section} placeholder="No more than 100 characters"/>
            </div>
          </div>
...
export default NovelEditDescription;
